...
# alpha = 0.0001
# lr = 0.2
cl_list = features_df['cycle_life']
ll_cells = []
for key, value in cl_list.iteritems():
    if value > 1000:
        ll_cells.append(key)

bad_cells = [90,118,123]
all_cells = range(0,124,1)
selected_cells = ll_cells

for index, row in features_df.iteritems():
    if (index != 'cell_key') and (index != 'cycle_life') and (index != 'cycle_550_clf'):
        print(f"{index}: {np.corrcoef(row[selected_cells],np.log(features_df['cycle_life'][selected_cells]))[0,1]}")
