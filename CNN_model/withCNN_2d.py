# -*- coding: utf-8 -*-
"""
Created on Mon Jul  4 18:17:27 2022
...
class ClassCNN(nn.Module):
    def __init__(self):
        ...
        # 把一张图打成10张卷积后的图
        self.conv1 = nn.Conv2d(1, 6, kernel_size=9) #1为输入通道数，2为输出通道数
        # 把10张图打成20张卷积后的图
        self.conv2 = nn.Conv1d(6, 6, kernel_size=10) 
        self.fc1 = nn.Linear(498, 100)
...
def scaler_3d_fit(data):
    data = data.reshape((data.shape[0],data.shape[1]*data.shape[2]))
    scaler = MinMaxScaler()
    scaler.fit(data)
    return scaler

def scaler_3d_transform(data,scaler):
    samples = data.shape[0]
    cols = data.shape[1]
    raws = data.shape[2]
    data = data.reshape((samples,cols*raws))
...
if __name__ == '__main__':
    ...
    #加载数据
    # delta_Q100_10_raw = np.zeros((len(dataset),9,1000))
    delta_Q100_10 = np.zeros((len(dataset),9,100))
    ...
    for i, cell in enumerate(dataset.values()):
        ...
        delta_Q100_10[i,0] = (cell['cycles']['100']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,1] = (cell['cycles']['90']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,2] = (cell['cycles']['80']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,3] = (cell['cycles']['70']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,4] = (cell['cycles']['60']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,5] = (cell['cycles']['50']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,6] = (cell['cycles']['40']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,7] = (cell['cycles']['30']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        delta_Q100_10[i,8] = (cell['cycles']['20']['Qdlin'] - cell['cycles']['10']['Qdlin'])[range(1,1000,10)]
        label[i] = np.log(cell['cycle_life'])
    ...
    input_cols = ic_train_valid.shape[1]
    input_raws = ic_train_valid.shape[2]
    output_size = label_train_valid.shape[-1]#label的第二维的大小即为神经网络输出层的神经元数
    # 归一化
    # scaler_X = scaler_3d_fit(np.concatenate((ic_train_valid,ic_test)))
    # ic_train_valid = scaler_3d_transform(ic_train_valid,scaler_X)
    ic_train_valid.resize((train_valid_samples,1,input_cols,input_raws))
    ...
    # 对测试集做归一化
    test_X = ic_test
    # test_X = scaler_3d_transform(ic_test,scaler_X)
    test_y = scaler_y.transform(label_test)
    test_X.resize((test_samples,1,input_cols,input_raws))
    #存入GPU
    ...
    # 反归一化
    ytest = np.exp(scaler_y.inverse_transform(ytest))
    test_pred = np.exp(scaler_y.inverse_transform(test_pred))
    ...
    # 反归一化
    ytrain = np.exp(scaler_y.inverse_transform(ytrain))
    train_pred = np.exp(scaler_y.inverse_transform(train_pred))
    ...
    # 反归一化
    yvalid = np.exp(scaler_y.inverse_transform(yvalid))
    valid_pred = np.exp(scaler_y.inverse_transform(valid_pred))
    ...
    plt.plot([np.exp(min_CAP),np.exp(max_CAP)],[np.exp(min_CAP),np.exp(max_CAP)],color='k')
    plt.legend()
    plt.xlabel('Real Life (CYC)')
    plt.ylabel('Predict Life (CYC)')
    plt.grid()
